...
$rocket_cache_filepath = $request_uri_path . '/' . $filename . '.php';
...
function do_rocket_callback( $buffer ) {
	...
	if ( strlen( $buffer ) > 255
		...
		if ( apply_filters( 'do_rocket_generate_caching_files', true ) ) {
			...
			// Save the cache file.
			rocket_put_content( $rocket_cache_filepath, apply_filters( 'rocket_put_cache', $buffer . $footprint ) );

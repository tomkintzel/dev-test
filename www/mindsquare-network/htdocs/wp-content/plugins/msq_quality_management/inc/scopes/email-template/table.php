...
class Table extends \MSQ\Plugin\Quality_Management\Scopes\Table {
	...
	public function load_page() {
		...
		add_screen_option( 'per_page', [
			'label' => 'Anzahl Email-Templates',
			'default' => 50,
			'option' => 'email_templates_per_page'
		]);
	...
	public function process_actions( $action ) {
		...
		if( $action == 'update' && !empty( $_REQUEST[ 'email_template' ] ) ) {
			$email_template_ids = $_REQUEST[ 'email_template' ];
			...
			$pardot_model_updater->update_email_template_index( isset( $_POST[ 'all_pages' ] ) );
			$pardot_model_updater->save();
			$pardot_model_updater->launch();
			...
			$pardot_model_updater->update_email_template_index();
			$pardot_model_updater->save();
			$pardot_model_updater->launch();
	...
	/**
	 * Erstellt eine Liste von Email-Templates.
	 */
	public function prepare_items() {
		...
		$dataset->set_values( Pardot_Model_Collection::get_instance()->get_email_templates() );
		...
		// Pagination
		$per_page = $this->get_items_per_page( 'email_templates_per_page' );
	...
	public function sort_items( $items ) {
		$orderby = $_GET[ 'orderby' ] ?? 'email_template_edit_date';
	...
	public function column_cb( $item ) {
		$email_template_id = $item->get_value( 'email_template_id' );
		?>
			<input id="cb-select-<?php echo $email_template_id; ?>" type="checkbox" name="email_template[]" value="<?php echo $email_template_id; ?>" />
		<?php
	}
	 ...
	 * @param Iterator $item
	 */
	function column_email_template_name( $item ) {
		// vars
		$email_template = $item->current();
		$actions[ 'edit' ] = sprintf( '<a href="%s">Email-Template Bearbeiten</a>', $email_template->get_read_url() );
		$pardot_model_updater = Pardot_Model_Updater::get_instance();

		// Wenn der Updater gerade keine Aufgaben macht
		if( !$pardot_model_updater->is_locked() ) {
			$actions[ 'update' ] = sprintf( '<a href="%s">Änderungen laden</a>', add_query_arg( [ 'email_template' => [ $email_template->get_id() ], 'action' => 'update' ], $_SERVER[ 'REQUEST_URI' ] ) );
		}

		// Ausgabe
		echo sprintf( '%s %s', $item->get_output( 'email_template_name' ), $this->row_actions( $actions ) );
	}
	...
	/**
	 * Diese Fehlermeldung wird angezeigt, wenn keine Email-Templates hinzugefügt wurden.
	 */
	public function no_items() {
		echo 'Keine Email-Templates gefunden';
	}

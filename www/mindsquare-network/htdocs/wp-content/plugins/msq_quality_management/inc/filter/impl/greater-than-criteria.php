...
class Greater_Than_Criteria implements Criteria {
	/** @var mixed $value */
	private $value;
	...
	public function debug( Criteriable $field ) {
		$values = $field->get_value( $this->fieldname );
		return sprintf( '(<abbr title="%s">%s</abbr> ist größer als "%s")', htmlspecialchars( implode( ', ', (array)$values ) ), $field->get_headline( $this->fieldname ), $this->value );
	}

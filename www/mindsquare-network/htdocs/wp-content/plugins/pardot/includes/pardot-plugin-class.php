...
class Pardot_Plugin {
	...
	function wp_ajax_popup_reset_cache() {
		// @see https://gitlab.com/mindsquare/mindsquare-network/issues/1236
		switch_to_blog(37);
		...
		if ( isset( $_GET['pardot-contact-request'] ) ) {
			...
			if ( ! $form_html && $forms = get_pardot_forms() ) {
				...
				if ( isset( $forms[$form_id] ) ) {
					...
					$result_set_transient = set_transient( 'pardot_form_html_' . $form_id, $form_html, self::$cache_timeout );

					// @see https://gitlab.com/mindsquare/mindsquare-network/issues/1236
					restore_current_blog();

					if ( $result_set_transient ) {
			...
			$querystring = isset($args['querystring']) ? $args['querystring'] : '';
			/**
			 * Allow programmatic modification of the querystring
			 */
			$args['querystring'] = apply_filters( 'pardot_form_querystring', $querystring );
	...
	static function convert_embed_code_https( $embed_code ) {
		/**
		 * Diese Funktion wird nicht mehr benötigt, da bei https://go.pardot.com/ die neue Funktion 'Tracking-Domains'
		 * hinzuegfügt wurde. Damit kann die Domain bei den Pardot-Einstellungen verändert werden.
		 */
		return $embed_code;
		...
		if ( Pardot_Settings::get_setting( 'https' ) ) {
			...
			$httpsurl = 'https://www2.mindsquare.de' . $urlpieces['path'];
	...
	static function get_dynamic_content_body( $args = array() ) {
		...
		if ( false === $dynamicContent_html ) {
			...
			$result_set_transient = set_transient( 'pardot_dynamicContent_html_' . $dynamicContent_id, $dynamicContent_html, self::$cache_timeout );
	...
	public static function set_cache( $key, $value, $set_transient = true ) {
		...
		$result_set_transient = set_transient( "pardot_{$key}", $value, self::$cache_timeout );
	...
	public static function save_transient_key( $key ) {
		...
		$result = update_option( self::$saved_transient_keys, array_values( $saved_trans ) );
		// @see https://gitlab.com/mindsquare/mindsquare-network/issues/1236
		restore_current_blog();
	...
	public static function save_cache_key( $key ) {
		...
		$result = update_option( self::$saved_cache_keys, array_values( $saved_keys ) );
		// @see https://gitlab.com/mindsquare/mindsquare-network/issues/1236
		restore_current_blog();

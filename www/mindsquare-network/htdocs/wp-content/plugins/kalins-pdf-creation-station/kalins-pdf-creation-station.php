...
function kalins_pdf_admin_init(){
  ...
  add_meta_box( "kalinsPDFNavMenu", "PDF Creation Station", 'kalinsPDF_nav_menu_box', 'nav-menus', 'side');
	...
	add_meta_box( 'kalinsPDF_sectionid', __( "PDF Creation Station", 'kalinsPDF_textdomain' ), 'kalinsPDF_inner_custom_box', 'expertise', 'side' );
  //--------------------------------
}
...
function kalinsPDF_inner_custom_box($post) {//creates the box that goes on the post/page edit page
  ...
  if($meta){//if that meta exists, set $showLink
	...
	switch($showLink){//KLUDGE - show radio buttons depending on which one is selected (there should be an easier way than repeating all that HTML - I mean, what if I had like 15 different options?)
		case "none":
			echo '<p><input type="radio" name="kalinsPDFLink" value="none" id="opt_none" checked /> Do not generate PDF<br /><input type="radio" name="kalinsPDFLink" value="show" id="opt_show" /> Add download link<br /><input type="radio" name="kalinsPDFLink" value="default" id="opt_default" /> Use default</p>';
			break;
		case "show":
			echo '<p><input type="radio" name="kalinsPDFLink" value="none" id="opt_none" /> Do not generate PDF<br /><input type="radio" name="kalinsPDFLink" value="show" id="opt_show" checked /> Add download link<br /><input type="radio" name="kalinsPDFLink" value="default" id="opt_default" /> Use default</p>';
			break;
		case "default":
			echo '<p><input type="radio" name="kalinsPDFLink" value="none" id="opt_none" /> Do not generate PDF<br /><input type="radio" name="kalinsPDFLink" value="show" id="opt_show" /> Add download link<br /><input type="radio" name="kalinsPDFLink" value="default" id="opt_default" checked /> Use default</p>';
			break;
	}
}
...
function kalinsPDF_publish_post( $post_id ){
	...
	$fileName = get_kalins_filepath( $post_id, $adminOptions );
    if(file_exists( $fileName )){//if the pdf file for this page already exists,
        unlink( $fileName );//delete it cuz it's now out of date since we're saving new post content
    }
...
function kalinsPDF_content_filter($content){
	...
	global $kalinsPDFRunning, $post, $wp_query;
	if( isset( $kalinsPDFRunning ) ) {
		return $content;
	}

	$adminOptions = kalins_pdf_get_options( KALINS_PDF_ADMIN_OPTIONS_NAME );
	//if we're not on a single page/post we don't need to do anything else
	$postType = get_post_type();
	if( false === apply_filters('kalinsPDF_show_Content', is_single() && $postType == 'post', $postType)  ) {
		return $content;
	}
  // Wenn der Inhalt nicht von der Main-Query geladen wird
  if( !empty( $wp_query->posts ) && $wp_query->posts[0]->ID != $post->ID ) {
    return $content;
  }
  ...
  $adminOptions->buttonText = kalins_pdf_page_shortcode_replace( $adminOptions->buttonText, $post );
  $adminOptions->boxHeadline = kalins_pdf_page_shortcode_replace( $adminOptions->boxHeadline, $post );
  $adminOptions->boxText = kalins_pdf_page_shortcode_replace( $adminOptions->boxText, $post );

  if( !empty( $adminOptions->pardotForm ) ) {
	$postTitle = get_the_title( $post );
	$postLink = get_the_permalink( $post );
	$pardot = msq_get_pardot_form( array(
		'form_id' => $adminOptions->pardotForm,
		'height' => '300px',
		'querystring' => http_build_query(array(
			'redirect' => get_bloginfo( 'wpurl' ) . '/wp-content/plugins/kalins-pdf-creation-station/kalins_pdf_create.php?singlepost=' . $postID,
			'Post_Title' => $postTitle,
			'Post_Link' => $postLink,
      'heruntergeladene_blogbeitraege' => $postLink,
      'eventCategory' => 'PDF Download',
      'eventLabel' => $postTitle
		))
	));
	/*if( preg_match( '/src="([^"]*)"/i', $pardot, $match ) ) {
		$pardot = preg_replace( '/src="([^"]*)"/i', '', $pardot );
		$pardot_src = $match[ 1 ];
	}*/
  }

  $html = '<a name="download-pdf"></a>';

  $html .= apply_filters( 'msq_kalinsPdfBeforeHtml', $html );
  $classes = apply_filters ( 'msq_kalinsPdfBtnClass', 'btn_conversion' );

  if( !empty( $pardot ) ) {
	wp_enqueue_script( 'bootstrap-modal' );
	$html .= '
		<div id="msq-kalins-tcpdf">
			<button type="button" class="' . $classes . '" data-toggle="modal" data-target="#kalinsTcpdfModal">' . $adminOptions->buttonText . '</button>
			<div class="modal fade" id="kalinsTcpdfModal" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content">
            <div class="modal-header">
              <div>
                <h3>' . $adminOptions->boxHeadline . '</h3>' .
                ( !empty( $adminOptions->boxText ) ? '<p>' . $adminOptions->boxText . '</p>' : '' ) .'
              </div>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
						<div class="modal-body"><div class="pardotwrapper">' . $pardot . '</div></div>
					</div>
				</div>
			</div>
		</div>';
		/*<script>
			jQuery(document).ready(function($) {
				var initModal = function() {
					var modal = $($("button[data-toggle=\'modal\']").attr("data-target"));
					modal.find(".pardotform").attr("src", "' . $pardot_src . '");
					$(this).unbind("click", initModal);
				}
				$("#msq-kalins-tcpdf button[data-toggle=\'modal\']").click(initModal);
			});
		</script>';*/
  }
  else {
	$html = '
		<div id="msq-kalins-tcpdf">
			<h3>' . $adminOptions->boxHeadline . '</h3>' .
			( !empty( $adminOptions->boxText ) ? $adminOptions->boxText : '' ) .
			'<div class="btn-wrapper"><a class="btn_conversion" href="' . get_bloginfo( 'wpurl' ) . '/wp-content/plugins/kalins-pdf-creation-station/kalins_pdf_create.php?singlepost=' . $postID . '" target="_blank" rel="noopener">' . $adminOptions->buttonText . '</a></div>
		</div>';
  }
  return $content . $html;
}
...
function kalinsPDF_build_pdf( $post ){
  $fileName = get_kalins_filepath( $post );

  if( $fileName ){//if the pdf file for this page already exists,
    return false;
  }
...
function kalins_pdf_getAdminSettings(){//simply returns all our default option values for settings page
  ...
  $kalinsPDFAdminOptions->headerTitle = '[post_title]';
  $kalinsPDFAdminOptions->headerSub = '[blog_name] - [blog_description]';
  $kalinsPDFAdminOptions->beforePage = '<h1>[post_title]</h1>';
  $kalinsPDFAdminOptions->beforePost = '<h1>[post_title]</h1>';
  $kalinsPDFAdminOptions->afterPage = '';
  $kalinsPDFAdminOptions->afterPost = '';
  $kalinsPDFAdminOptions->titlePage = '';
  $kalinsPDFAdminOptions->finalPage = '';
  $kalinsPDFAdminOptions->includeImages = true;
  ...
  $kalinsPDFAdminOptions->autoGenerate = true;
  $kalinsPDFAdminOptions->showLink = "none";
  $kalinsPDFAdminOptions->filenameByTitle = true;
  $kalinsPDFAdminOptions->pardotForm = 0;
  $kalinsPDFAdminOptions->boxHeadline = 'Beitrag als PDF-Dokument downloaden';
  $kalinsPDFAdminOptions->boxText = '';
  $kalinsPDFAdminOptions->buttonText = '<i class="fa fa-download" aria-hidden="true"></i> Als PDF Herunterladen';
  $kalinsPDFAdminOptions->doCleanup = true;
  $kalinsPDFAdminOptions->wordCount = 0;

  // Ansprechpartner
  $fb_img = get_field( 'fb_img', 'option' );
  if( !empty( $fb_img ) ) {
    $kalinsPDFAdminOptions->contactImg = $fb_img[ 'url' ];
  }
  $kalinsPDFAdminOptions->contactName = get_field( 'fb_name', 'option' );
  $kalinsPDFAdminOptions->contactPosition = get_field( 'fb_function', 'option' );
  $kalinsPDFAdminOptions->contactTel = get_field( 'fb_phone', 'option' );
  $kalinsPDFAdminOptions->contactEmail =  get_field( 'fb_email', 'option' );
  $kalinsPDFAdminOptions->blogName = get_option( 'blogname' );
  $kalinsPDFAdminOptions->blogUrl = get_option( 'home' );

  $kalinsPDFAdminOptions->titlepageBGImg = '';
...
function kalins_pdf_getToolSettings(){//simply returns all our default option values for tool page
  $kalinsPDFAdminOptions = new stdClass();
  $kalinsPDFAdminOptions->headerTitle = '[blog_name]';
  $kalinsPDFAdminOptions->headerSub = '[blog_description]';
  $kalinsPDFAdminOptions->beforePage = '<h1>[post_title]</h1>';
  $kalinsPDFAdminOptions->beforePost = '<h1>[post_title]</h1>';
  $kalinsPDFAdminOptions->afterPage = '';
  $kalinsPDFAdminOptions->afterPost = '';
...
class WP_Kalins_PDF_Creation_Station_Widget extends WP_Widget {
  ...
  function widget($args, $instance) {
    ...
    if(empty($instance['pardotForm'])){
      $adminOptions->pardotForm = kalins_pdf_page_shortcode_replace($adminOptions->pardotForm, $post);
    }else{
      $adminOptions->pardotForm = kalins_pdf_page_shortcode_replace($instance["pardotForm"], $post);
    }

    if(empty($instance['boxText'])){
      $adminOptions->boxText = kalins_pdf_page_shortcode_replace($adminOptions->boxText, $post);
    }else{
      $adminOptions->boxText = kalins_pdf_page_shortcode_replace($instance["boxText"], $post);
    }

    if(empty($instance['buttonText'])){
      $adminOptions->buttonText = kalins_pdf_page_shortcode_replace($adminOptions->buttonText, $post);
    }else{
      $adminOptions->buttonText = kalins_pdf_page_shortcode_replace($instance["buttonText"], $post);
    }
  ...
  function form($instance) {
    ...
    $aTextFieldLabels = array( "Titel:", "Überschrift der Download-Box:", "Text der Download-Box:", "Oberer Text:", "Unterer Text:" );
    $aTextFieldNames = array( "title", "boxHeadline", "boxText", "buttonText" );
    $adminOptions = kalins_pdf_get_options(KALINS_PDF_ADMIN_OPTIONS_NAME);
    $aTextFieldDefaultValues = array("", $adminOptions->boxHeadline, $adminOptions->boxText, $adminOptions->buttonText);
...
// Register the widget.
function kalins_pdf_register_widget () {
  return register_widget("WP_Kalins_PDF_Creation_Station_Widget");
}
add_action( 'widgets_init', 'kalins_pdf_register_widget' );
...
/**
 * Gibt den Dateinamen der PDF für ein Beitragsobjekt zurück.
 * Enthält keine Dateiendung.
 *
 * @param int           $postID  Die Beitrags-ID
 * @param stdClass|null $options Die Admin-Optionen des Plugins
 */
function get_kalins_filename( $postID, $options = null ) {
	$filename = $postID;

	if ( empty( $options ) ) {
		$options = kalins_pdf_get_options( KALINS_PDF_ADMIN_OPTIONS_NAME );
	}

	if ( $options->filenameByTitle ) {
...
function create_kalins_filedir( $postID, $options = null ) {
	$dir = KALINS_PDF_SINGLES_DIR . get_kalins_filedir( $postID, $options );
...
function get_kalins_filedir( $postID, $options = null ) {
	$dir = '';
	...
	if ( $options->filenameByTitle ) {
		$dir = get_post_type( $postID ) . '/';
	}

	return $dir;
}

function get_kalins_filepath( $postID, $options = null) {
	$path = KALINS_PDF_SINGLES_DIR
	        . get_kalins_filedir( $postID, $options )
	        . get_kalins_filename( $postID, $options ) . '.pdf';
	return $path;
}

function get_kalins_fileurl( $postID, $options = null ) {
	$url = KALINS_PDF_SINGLES_URL
	       . get_kalins_filedir( $postID, $options )
	       . get_kalins_filename( $postID, $options ) . '.pdf';

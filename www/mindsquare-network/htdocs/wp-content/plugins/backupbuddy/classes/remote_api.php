	...
	public static function remoteCall( $remoteAPI, $verb, $moreParams = array(), $timeout, $files = array(), $returnRaw = false ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( false === ( $return = @unserialize( $response['body'] ) ) ) {
				$error = "Error #8001: Unable to decode Deployment response. Things to check:\n\n Verify both sites are running the latest BackupBuddy version (v8.1.1.1 introduced non-backward-compatible changes).\n Check the remote site API URL is correct: " . $remoteAPI['siteurl'] . ".\n If you changed the remote site API key you must update it into this site\n Make sure the remote site has the API enabled in its wp-config.php by adding define( 'BACKUPBUDDY_API_ENABLE', true ); somewhere ABOVE the line `That's all, stop editing!`.\n\n Verb: `" . $verb . "`.\n\n Return data: `" . $sanibody . "`.\n\n Troubleshooting: `<textarea>" . htmlentities( print_r( $response, true ) ) . "</textarea>`.";
		...
		foreach( pb_backupbuddy::$options['remote_api']['keys'] as $key ) {
			...
			if ( $key_public == $keyArr['key_public'] ) { // Incoming public key matches a stored public key.
				...
				if ( $calculatedSignature != $signature ) { // Key matched but signature failed. Data has been tempered with or damaged in transit.
					if ( pb_backupbuddy::$options['log_level'] == '3' ) { // Full logging enabled.
						pb_backupbuddy::status( 'details', 'Deployment incoming call: Key signature match failed.' );
					}
					return false;
				} else { // Signature good.
					if ( pb_backupbuddy::$options['log_level'] == '3' ) { // Full logging enabled.
						pb_backupbuddy::status( 'details', 'Deployment incoming call: Signature good.' );
					}
					
					if ( false === ( self::$_incomingPayload = @unserialize( $_incomingPayload ) ) ) { // Corrupt payload.
						if ( pb_backupbuddy::$options['log_level'] == '3' ) { // Full logging enabled.
							pb_backupbuddy::status( 'details', 'Deployment incoming call: Payload corrupt/undecodable.' );
						}

...
class backupbuddy_core {
	...
	public static function get_directory_exclusions( $profile, $trim_suffix = true, $serial = '' ) {
		...
		if ( $trim_suffix ) {
			array_walk( $excludes, function( &$val ) { // Apply trim to all items within.
				$val = rtrim( trim( $val ), '/' );
			});
		} else {
			array_walk( $excludes, function( &$val ) { // Apply (whitespace-only) trim to all items within.
				$val = trim( $val );
			});
		}
	...
	public static function get_mysqldump_additional( $includesOrExcludes, $profile, $override_prefix = '' ) {
		...
		array_walk( $tables, function( &$val ) { // Trim whitespace around tables.
			$val = trim($val);
		});

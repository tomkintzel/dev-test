...
function MRP_inner_custom_box() {
	...
	foreach($post_types as $post_type) {
	    ...
	    $ext = "-".$n;
		$post_type_details = get_post_types( array( 'name' => $post_type ), 'objects' );
	    $post_type_details = array_shift( $post_type_details );
	    $current = ($n==1) ? ' MRP_current_tab' : '';
	    $related_posts_count = empty( $related_posts[ $post_type ] ) ? 0 : count( $related_posts[ $post_type ] );
	...
	foreach($post_types as $post_type) {
		...
		if( $post_ID ) {
			if( !empty( $related_posts[ $post_type ] ) && count( $related_posts[ $post_type ] ) > 0 ) {
